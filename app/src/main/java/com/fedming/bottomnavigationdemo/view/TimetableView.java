package com.fedming.bottomnavigationdemo.view;
...
public class TimetableView extends ViewGroup {
    ...
    public List<CourseModel> mList;

    private static final int MAX_DAY = 7; //一周 7 天

    private static final int MAX_SECTION = 9; //一天 9 节课
    ...
    private OnCourseItemClickListenerImp mListener;
    ...
    public TimetableView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        setOnCourseItemClickListener(new OnCourseItemClickListenerImp());
    }
...
//                        调用实现的方法！
    ...
    public void setOnCourseItemClickListener(OnCourseItemClickListenerImp listener) {
        mListener = listener;
    }
    ...
    public class OnCourseItemClickListenerImp implements OnCourseItemClickListener {

        @Override
        public void onCourseItemClick(CourseModel course) {
            Toast.makeText(context, course.getCname(), Toast.LENGTH_SHORT).show();
        }
